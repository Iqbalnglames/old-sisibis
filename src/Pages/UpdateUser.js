...
const UpdateUser = ({ showUpdateUser, data }) => {
  ...
  const registrasi = async (e) => {
    ...
    await axios.patch("http://127.0.0.1:8000/api/register", formData)
      .then((response) => {
      ...
      <div className="flex justify-center shadow-lg bg-slate-300 mx-[65vh] p-4 rounded-md">
        <form onSubmit={registrasi} encType="multipart/form-data" className="text-base">
          ...
          <input
            ...
            className={inputStyle}
            onChange={(e) => setUsername(e.target.value)} value={data.username}
            />
            {
                             validation.username && (
                                 <div className={alertStyle}>
